...
function main() {
	...
	program
		...
		.option('-i, --bridgeID <string>', '(Optional) The bridge-id on the network. Generates a random bridge-id string if blank. Example: -i myBridge0')
		.option('-b, --brokerIP <string>', '(Optional) The IP address of the MQTT broker. Defaults to localhost if blank. Example: -b 192.168.0.123')
	...
	this.mqttClient = mqtt.connect(brokerIP, { protocolVersion: 5 });
	...
	bridge = new dabBridge(bridgeID, target, brokerIP);
}
